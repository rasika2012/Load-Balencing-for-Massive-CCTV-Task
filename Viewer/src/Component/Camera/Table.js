...
export class Table extends Component {
    ...
    createData(id, ip, addedDate, status, link,  additional) {
        return { id, ip, addedDate, status, link, additional };
    }
    ...
    setData(data) {
        ...
        for (i = 0; i < data.length; i++) {
          row.push(this.createData(data[i][0],data[i][1],data[i][2],data[i][3],data[i][4],data[i][5]));
        } 
        this.setState({
            value:row,
            isLoaded: true,
